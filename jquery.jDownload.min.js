(function($){$.fn.jDownload=function(settings){var config={root:"/",filePath:null,event:"click",dialogTitle:"jDownload",dialogDesc:$(this).attr('title')?$(this).attr('title'):'Download the file now?',dialogWidth:400,dialogHeight:'auto',dialogModal:true,showfileInfo:true,start:null,stop:null,download:null,cancel:null}
...
dialog.html("");var filePath=(settings.filePath==null)?$(this).attr('href'):settings.filePath;dialog.html('<p>Fetching File...</p><img src="'+settings.root+'jDownload/loader.gif" alt="Loading" />');$.ajax({type:'GET',url:settings.root+'jDownload/jDownload.php',data:'action=download&path='+filePath,success:function(data){setTimeout(function(){if(data=="error"){dialog.html("<p class=\"jDownloadError\">File cannot be found.</p>");}else{if(settings.showfileInfo==true){var url=settings.root+'jDownload/jDownload.php?action=info&path='+filePath;$.getJSON(url,function(data){var html="<div class=\"jDownloadInfo\">";html+="<p><span>File Name:</span> "+data.filename+"</p>";html+="<p><span>File Type:</span> "+data.filetype+"</p>";html+="<p><span>File Size:</span> "+data.filesize+" KB</p>";html+="</div>";$('.jDownloadInfo, .jDownloadError').remove();dialog.html('<p>'+settings.dialogDesc+'</p>'+html);});}}},200);}});dialog.data('jDownloadData',{filePath:filePath}).dialog('open');return false;});function start_download(i){iframe.attr('src',settings.root+'jDownload/jDownload.php?action=download&path='+dialog.data('jDownloadData').filePath);dialog.dialog('close');return false;}}})(jQuery);